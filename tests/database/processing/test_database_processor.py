...
def test_processed_info():
    ...
    assert result["op.stock"] is not None
    assert result["op.stock"]["schema"] == "op"
    assert result["op.stock"]["name"] == "stock"

    assert result["op.employee"] is not None
    assert result["op.employee"]["schema"] == "op"
    assert result["op.employee"]["name"] == "employee"
...
def test_processed_estimated_rows():
    ...
    assert result["public.stock"]["estimated_rows"] == 203
    assert result["public.user"]["estimated_rows"] == 1
    assert result["public.employee"]["estimated_rows"] == 54
